...
}:
{
  ...
  programs.zsh.shellAliases = {
    ...
    ghbv = "gh run view $(gh run list --workflow build.yml -L 1 --json databaseId,conclusion,status \
  --jq '[.[] | select(.conclusion == \"failure\" or .conclusion == \"success\" or .status == \"in_progress\")] | .[].databaseId')";
